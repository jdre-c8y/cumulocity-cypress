...
- [B2's first run: the probe never reached the widget configuration](issues/19-b2-first-run.md) — FAIL, $4.67, no spec; findings 1 and 2 built (2026-09-15), 3 is a caution needing no code, 4 a confirmation. Re-run (`b2-second`) still FAIL, $6.96 — findings 1/2 held, but the run exhausted its probe budget guessing a wrong modal selector for a click/settle target, a gap finding 1's scope didn't reach. Finding 5 closed that gap (2026-09-16); `b2-third` caught a bug in finding 5 itself (`cy.wrap()` on an empty jQuery collection re-triggers Cypress's own retry-then-throw), corrected, and `b2-fourth` **PASSED** — axis A and B automated-PASS, 6/6 outcomes, $5.91, axes C/D pending human grading. The ancestor-scoped position rung gap turned out not to be needed once probe budget wasn't wasted on the click hazard.
